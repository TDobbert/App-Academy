...
def filter_out(array, &prc)
    ...
    array.each do |nums|
        filtered << nums if !prc.call(nums)
    end
    filtered
end
...
def xnor_select(array, prc1, prc2)
    array.select do |nums|
        nums if prc1.call(nums) && prc2.call(nums) || !prc1.call(nums) && !prc2.call(nums)
    end
end
...
def filter_out!(array, &prc)
    array.select! {|nums| nums if !prc.call(nums)}
end
...
#p multi_map([4, 3, 2, 7], 4) { |num| num * 10 }             # [40000, 30000, 20000]
...
def proctition(array, &prc)
    part1 = array.select {|ele| ele if prc.call(ele)}
    part2 = array.select {|ele| ele if !prc.call(ele)}
...
#hash contains procs as keys and suffix strings as values. The method should 
...
#1, 2, and 3. 
#The values associated with each key should be an array containing 
...
def proctition_platinum(array, *prcs)
    ...
    prcs.each_index {|i| proctitions[i + 1] = []}
...
#sentence where each word of the input sentence is changed by a value proc if 
#the original word returns true when passed into the key proc. If an original 
...
def procipher(sentence, ciphers)
    words = sentence.split(" ")

    new_words = words.map do |word|
        new_word = word
        ciphers.each do |prcs, ci_prcs|
            new_word = ci_prcs.call(new_word) if prcs.call(word)
        end
        new_word
    end
    new_words.join(" ")
...
#sentence where each word of the input sentence is changed by a value proc if 
#the original word returns true when passed into the key proc. If an original 
#word returns true for multiple key procs, then only the value proc that appears 
#earliest in the hash should be applied.

def picky_procipher(sentence, ciphers)
    words = sentence.split(" ")
